...
public class OddStringDifference {
    ...
    class Solution {
        public String oddString(String[] words) {
            String oddA = null, oddB = null;
            Integer indexOddA = -1, indexOddB = -1;
            for (int i = 0; i < words.length; i++) {
                StringBuilder currentStr = new StringBuilder();
                for (int j = 0; j < words[i].length() - 1; j++) {
                    currentStr.append(words[i].charAt(j + 1) - words[i].charAt(j)).append(",");
                }
                if (oddA == null){
                    oddA = currentStr.toString();
                    indexOddA = i;
                }else if (oddB == null){
                    oddB = currentStr.toString();
                    indexOddB = i;
                }else{
                    if (oddA.equals(oddB) && oddB.equals(currentStr.toString())){
                        continue;
                    }else{
                        if (oddA.equals(oddB)){
                            return words[i];
                        }else if (oddA.equals(currentStr.toString())){
                            return words[indexOddB];
                        }else{
                            return words[indexOddA];
                        }
                    }
                }
            }
